...
export function TimesheetEntryForm({ onCopyEntry, copyData, onCopyDataConsumed }: TimesheetEntryProps) {
  ...
  // 常用模版状态
  const [userTemplates, setUserTemplates] = useState<TimesheetTemplate[]>([]);
  ...
  const handleSaveAsTemplate = async () => {
    ...
    template.fields.forEach(field => {
      if (field.key !== 'hours' && formData[field.key]) {
  ...
  // 应用模版
  const handleApplyTemplate = (tpl: TimesheetTemplate) => {
                        ...
                        <div className="p-2 border-b border-slate-100 bg-slate-50/50">
                          <div className="flex items-center justify-between">
                            ...
                            <button
                              ...
                              className="flex items-center gap-1 px-2 py-1 rounded text-xs font-medium text-blue-600 hover:bg-blue-50 transition-colors"
                            >
                          ...
                          {userTemplates.length === 0 ? (
                            ...
                            userTemplates.map((tpl) => (
                              <button
                                ...
                                className="w-full px-3 py-2.5 text-left hover:bg-slate-50 transition-colors group flex items-center justify-between"
                              >
                                <div className="flex-1 min-w-0">
                                  <div className="text-sm font-medium text-slate-700 truncate">{tpl.name}</div>
                                  <div className="text-xs text-slate-400 truncate mt-0.5">
                                    {Object.values(tpl.data).filter(v => v).slice(0, 2).join(' / ')}
                                  </div>
                                </div>
                                <button
                                  type="button"
                  ...
                  <div className="pt-2 space-y-2">
                    ...
                    {editingId ? (
                      ...
                      <Button 
                        ...
                        variant="outline"
                        onClick={resetForm}
                        className="w-full h-10 rounded-xl border-slate-200 bg-white/80 text-slate-500 font-medium hover:bg-red-50 hover:border-red-200 hover:text-red-600 transition-all duration-200"
                      >
                        <svg className="w-4 h-4 mr-2" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                ...
                <div className="p-3 rounded-xl bg-slate-50 border border-slate-100">
                  ...
                  <div className="flex flex-wrap gap-1.5">
                    {template.fields.map(field => {
                      const value = formData[field.key];
                      if (!value || field.key === 'hours') return null;
                      return (
                        <span key={field.key} className="inline-flex items-center px-2 py-1 rounded-md bg-white border border-slate-200 text-xs text-slate-600">
                          {field.label}: <span className="font-medium ml-1 text-slate-800 truncate max-w-[100px]">{String(value)}</span>
                        </span>
                      );
                    })}
                  </div>

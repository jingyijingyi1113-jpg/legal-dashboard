...
def start_backup_scheduler():
    ...
    # 每周二上午 9 点检查并发送工时提醒（仅在每月第一周生效）
    schedule.every().tuesday.at("09:00").do(scheduled_timesheet_reminder)
    ...
    reminder_status = "已启用（每月第一周周二09:00）" if EMAIL_ENABLED else "未启用"
...
def send_simple_email(to_emails, subject, body):
    ...
    try:
        ...
        for i in range(0, len(to_emails), batch_size):
            ...
            if 'Bcc' in msg:
                del msg['Bcc']
            msg['To'] = EMAIL_SENDER  # To 填发件人自己
            msg['Bcc'] = ', '.join(batch)  # 收件人放到 BCC，互相不可见
...
def send_timesheet_reminder():
    ...
    # 英文月份名称
    last_month_name_en = last_month.strftime('%B %Y')
    
    subject = f"【工时提醒 Timesheet Reminder】请及时填写 {last_month_name} 工时记录"
...
The timesheet submission period for {last_month_name_en} has ended. Please complete and submit your timesheet within this week.

📌 Entry: https://www.ctdms.woa.com
📌 Deadline: Friday 18:00

Notes:
1. Please ensure the accuracy and completeness of your timesheet data
2. If you have any questions, please contact ivyjyding

If you have already submitted, please ignore this email.
Thank you for your cooperation!

This email is automatically sent by System. Please do not reply.
